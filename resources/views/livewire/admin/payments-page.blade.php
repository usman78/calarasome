<div class="mx-auto w-full max-w-7xl space-y-6">
    ...
    <div class="rounded-xl border border-zinc-200 bg-white p-4 shadow-xs dark:border-zinc-800 dark:bg-zinc-950">
        <div wire:loading.flex wire:target="clinicFilter,statusFilter" class="mb-3 items-center gap-2 rounded-lg bg-zinc-100 px-3 py-2 text-xs text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-zinc-200 text-left text-zinc-600 dark:border-zinc-700 dark:text-zinc-300">
                        ...
                        <th class="px-3 py-2 font-medium">Grace Expires</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($payments as $payment)
                        ...
                        <tr class="border-b border-zinc-100 align-top dark:border-zinc-800">
                            <td class="px-3 py-2 text-zinc-700 dark:text-zinc-200">{{ $payment['updated_at'] }}</td>
                            <td class="px-3 py-2 text-zinc-700 dark:text-zinc-200">{{ $payment['clinic'] }}</td>
                            ...
                            <td class="px-3 py-2 text-zinc-700 dark:text-zinc-200">{{ $payment['strategy'] }}</td>
                            <td class="px-3 py-2 text-zinc-700 dark:text-zinc-200">
                                {{ number_format($payment['amount'] / 100, 2) }} {{ strtoupper($payment['currency']) }}
                            </td>
                            <td class="px-3 py-2 text-zinc-700 dark:text-zinc-200">{{ $payment['provider'] }}</td>
                            <td class="px-3 py-2 text-zinc-700 dark:text-zinc-200">{{ $payment['appointment_type'] }}</td>
                            <td class="px-3 py-2 text-zinc-700 dark:text-zinc-200">
